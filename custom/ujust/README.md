...
**Do not install packages via $DNF_CMD/rpm in ujust commands.** Bootc images are immutable and package installation should happen at build time in [`build/build.sh`](../../build/build.sh).
...
2. If on a bootc system: `sudo bootc switch --target localhost/almalinux-atomic-desktop-tr:stable`
